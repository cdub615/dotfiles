...
export ZSH="/home/cdub/.oh-my-zsh"
...
zc () {
    ...
    jq -r '[.apiUrl // "", .apiKey // "", .yolo // false, .haikuModel // "", .sonnetModel // "", .opusModel // ""] | @tsv' "$config"
        ...
        ANTHROPIC_BASE_URL="$api_url" ANTHROPIC_AUTH_TOKEN="$api_key" ANTHROPIC_DEFAULT_HAIKU_MODEL="$haiku_model" ANTHROPIC_DEFAULT_SONNET_MODEL="$sonnet_model" ANTHROPIC_DEFAULT_OPUS_MODEL="$opus_model" claude $yolo_flag $dir_add "$@"
}
...
plugins=(git command-not-found docker npm zsh-interactive-cd zsh-navigation-tools archlinux zsh-syntax-highlighting zsh-autosuggestions)

# pnpm
export PNPM_HOME="/home/cdub/.local/share/pnpm"
case ":$PATH:" in
  *":$PNPM_HOME:"*) ;;
